...
int UVCPreview::setFrameCallback(jobject frame_callback_obj, int pixel_format) {
...
static void
copyFrame(const uint8_t *src, uint8_t *dest, const int width, int height, const int stride_src, const int stride_dest) {
...
uvc_frame_t *
UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func, int pixcelBytes) {
    // ENTER();
...
void UVCPreview::do_capture_idle_loop(JNIEnv *env) {
    ...
    for (; isRunning() && isCapturing();) {
        do_capture_callback(waitCaptureFrame());
    }
 ...
 */
void UVCPreview::do_capture_callback(uvc_frame_t *frame) {
    ENTER();
