...
int UVCCamera::setStatusCallback(jobject status_callback_obj) {
    ENTER();
    int result = EXIT_FAILURE;
    if (mStatusCallback) {
        result = mStatusCallback->setCallback(status_callback_obj);
    }
    RETURN(result, int);
}

int UVCCamera::setButtonCallback(jobject button_callback_obj) {
    ENTER();
    int result = EXIT_FAILURE;
    if (mButtonCallback) {
        result = mButtonCallback->setCallback(button_callback_obj);
    }
    RETURN(result, int);
}
...
int UVCCamera::setFrameCallback(jobject frame_callback_obj, int pixel_format) {
    ENTER();
    int result = EXIT_FAILURE;
    if (mPreview) {
        result = mPreview->setFrameCallback(frame_callback_obj, pixel_format);
    }
    RETURN(result, int);
}
...
int UVCCamera::updateAnalogVideoLockStateLimit(int &min, int &max, int &def) {
    ...
    if (mPUSupports & PU_AVIDEO_LOCK) {
        UPDATE_CTRL_VALUES(mAnalogVideoLockState, uvc_get_analog_video_lock_state)
    }
    RETURN(ret, int);
}
